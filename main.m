...
conf = struct(...
    ...
    'path', [pwd '\2-3D_task'],... %Путь к папке в которой будут хранится рпезультаты уточнения
    'kof', 0.75,... %масштабный множитель (исопльзуется дли изменения скорости сходимости). Может быть изменен по ходу уточнения в файле kof.temp
    'ch', 1,...  %максиамльное изменение в % жесткости на любой итерации
    'g_ch', 10,... %global changeable максиамльное изменение жесткости в % от первоначального значения (на первой итерации)
